...
/**
 * This file chooser extends the {@link javax.swing.JFileChooser} in a number of
 * ways.
 * <ul>
 * <li>Adds an extention to the filename based on the file filter, when the user
 * didn't specify any.</li>
 * <li>If the file to be saved is not accepted by the chosen file filter, it
 * confirms that the user really wants to do this. This is done because the same
 * file filter is used to determine with which plugin to load the file.</li>
 * <li>Confirms before overwriting an existing file.</li>
 * </ul>
 * Automatic adding of file extension only works with
 * {@link de.yaams.extensions.basemap.tiled.mapeditor.util.ConfirmableFileFilter}
 * .
 */
public final class ConfirmingFileChooser extends JFileChooser {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 66720667408823756L;
	...
	public void approveSelection() {
		...
		if ((lastDot == -1 || lastDot == filename.length() - 1) && getFileFilter() instanceof ConfirmableFileFilter) {
			ConfirmableFileFilter filter = (ConfirmableFileFilter) getFileFilter();
		...
		if (!getFileFilter().accept(file)) {
			int result = JOptionPane.showConfirmDialog(this, CONFIRM_MISMATCH, CONFIRM_MISMATCH_TITLE, JOptionPane.YES_NO_OPTION);
