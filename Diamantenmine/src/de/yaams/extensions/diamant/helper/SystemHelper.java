...
public class SystemHelper {
	 ...
	 *            for exist?
	 */
	// public static void runExternal(final String[] command, final boolean
	// wait) {
	...
	// YDialog.ok(Arrays.toString(command), "Exitcode:" + p.exitValue() + " " +
	// erg, p.exitValue() == 0 ? "info" : "error");
	// }
	// } catch (final Throwable t) {
	...
	public static void sendData(String type, String title, final HashMap<String, String> data) {
		...
		try {
			...
			data.put("type", type);
			data.put("yaams", Project.getName());
