...
import de.yaams.extensions.basemap.tiled.mapeditor.util.cutter.BasicTileCutter;
import de.yaams.extensions.basemap.tiled.util.Base64;
import de.yaams.extensions.basemap.tiled.util.Util;
...
public class XMLMapTransformer /* implements MapReader */{
	...
	private static String makeUrl(File filename) throws MalformedURLException {
		final String url;
		if (filename.getAbsolutePath().indexOf("://") > 0 || filename.getAbsolutePath().startsWith("file:")) {
	...
	private void reflectInvokeMethod(Object invokeVictim, Method method, String[] args) throws Exception {
		...
		for (int i = 0; i < parameterTypes.length; i++) {
			if ("int".equalsIgnoreCase(parameterTypes[i].getName())) {
				...
				Log.ger.debug("Unsupported argument type " + parameterTypes[i].getName() + ", defaulting to java.lang.String");
				conformingArguments[i] = args[i];
			}
		}
	...
	private void setOrientation(String o) {
		if ("isometric".equalsIgnoreCase(o)) {
			...
			Log.ger.warn("Unknown orientation '" + o + "'");
		}
	}
			...
			for (int i = 0; i < nnm.getLength(); i++) {
				...
				try {
					...
					if (j >= 0) {
						...
						Log.ger.warn("Unsupported attribute '" + n.getNodeName() + "' on <" + node.getNodeName() + "> tag");
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	private Image unmarshalImage(Node t, String baseDir) throws IOException {
		...
		if (source != null) {
			if (Util.checkRoot(source)) {
				source = makeUrl(new File(source));
			} else {
				source = makeUrl(new File(baseDir, source));
			}
			...
			for (int i = 0; i < nl.getLength(); i++) {
				...
				if ("data".equals(node.getNodeName())) {
					...
					if (cdata == null) {
						Log.ger.warn("image <data> tag enclosed no " + "data. (empty data tag)");
					} else {
	...
	public Map readMap(File filename) throws Exception {
		xmlPath = filename.getAbsolutePath().substring(0, filename.getAbsolutePath().lastIndexOf(File.separatorChar) + 1);
	...
	/**
	 * @see de.yaams.extensions.basemap.tiled.io.PluggableMapIO#getDescription()
	 */
