 ...
 * </pre>
 * 
 * And 'top' and 'bottom' result in hexes with the pointy sides to the left and
 * right and the first column either aligned top or bottom:
 * 
 * <pre>
 *   __
 *  /  \
...
public class HexMapView extends MapView {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -7184914195553515747L;
	...
	/*
	 * hexEdgesToTheLeft: This means a layout like this: __ / \ \__/ as opposed
	 * to this: /\ | | \/
	 */
	...
	/**
	 * The scroll increment when clicking in the trough of a scrollbar, that is
	 * scrolling a page. The amount is set to the size of the completely visible
	 * hexes in the viewport.
	 * 
	 * @param visibleRect
	 *            Current viewport rectangle.
	 * @param orientation
	 *            SwingConstants.VERTICAL or HORIZONTAL.
	 * @param direction
	 *            > 0 = scrolling down; < 0 = scrolling up.
	 * 
	...
	/**
	 * The scroll increment when clicking on the arrows of a scrollbar, that is
	 * scrolling one hex horizontically or vertically.
	 * 
	 * @param visibleRect
	 *            Current viewport rectangle.
	 * @param orientation
	...
	/**
	 * The total size of the viewport so that all tiles of the map fit in.
	 * 
	...
	public Dimension getPreferredSize() {
		...
		if (hexEdgesToTheLeft) {
			...
			h = map.getHeight() * (tsize.height + border) + (int) (tsize.height / 2 + 0.49) + border;
		} else {
			w = map.getWidth() * (tsize.width + border) + (int) (tsize.width / 2 + 0.49) + border;
	 ...
	 * 
	 * @param g2d
	 *            The graphics context, i.e. where to paint.
	 * @param layer
	 *            The layer to paint. Can be a special layer like the selection
	 *            layer.
	 */
	@Override
	...
	/**
	 * Paint one layer to the viewport. Is this function used? Not implemented!
	 * 
	 * @param g2d
	 *            The graphics context, i.e. where to paint.
	 * @param og
	...
	private Dimension getEffectiveTileSize() {
		...
		return new Dimension((int) (map.getTileWidth() * zoom + 0.999), (int) (map.getTileHeight() * zoom + 0.999));
	}
	...
	/**
	 * Paint coordinates. This should draw tiny coordinates in every hex tile.
	 * Not implemented.
	 * 
	 * @param g2d
	 *            The graphics context, i.e. where to paint.
	 ...
	 * 
	 * @param screenX
	 *            The x coordinate of a point in the viewport.
	 * @param screenY
	 *            The y coordinate of a point in the viewport.
	 * 
	...
	public Point screenToTileCoords(int screenX, int screenY) {
		...
		if (x < hWidth) {
			...
			if (hexEdgesToTheLeft) {
				col = (int) ((x - hWidth) / (double) (getThreeQuarterHex() + border) + 0.001);
			} else {
		...
		if (y < hHeight) {
			...
			if (hexEdgesToTheLeft) {
				...
				row = (int) ((y - hHeight) / (double) (getThreeQuarterHex() + border) + 0.001);
			}
		}
		...
		for (int i = 0; i < fourPoints.length; i++) {
			...
			// TiledLogger.getLogger().info("  min.pt " + fourPoints
			// [minI].getX()
	...
	/**
	 * Repaint a region of the viewport. This function has been disabled. I have
	 * tried it but it seems to repaint with some offset.
	 * 
	 * @param region
	 *            The rectangle of the viewport to be repainted.
	 */
	@Override
	public void repaintRegion(Rectangle region) {
		...
		// // This code should work. I've disabled it because of general
		// problems with the view refresh.
		// // Point2D topLeft=getTopLeftCornerOfTile((int)
		// region.getMinX(),(int) region.getMinY(),zoom);
		// // Point2D bottomRight=getTopLeftCornerOfTile((int)
		// region.getMaxX(),(int) region.getMaxY(),zoom);
		// Point2D topLeft=getTopLeftCornerOfTile((int) region.getMinX(),(int)
		// region.getMinY());
		// Point2D bottomRight=getTopLeftCornerOfTile((int)
		// region.getMaxX(),(int) region.getMaxY());
		//
	 ...
	 * @param border
	 *            0 = no border, 1 = with border line.
	 * 
