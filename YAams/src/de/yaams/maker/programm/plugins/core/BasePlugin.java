...
public abstract class BasePlugin {
	...
	public boolean isVersionInstall(String id, double minversion, double maxversion, YMessagesDialog mess) {
		...
		if (!PluginManager.isInstall(id)
			...
			// say it
			return PluginManager.installFromOnline(id);
		}

		double ver = PluginManager.getInfo(id).getVersion();

		// has the version?
		return internVersionCheck(PluginManager.getInfo(id).getTitle(), ver, minversion, maxversion, mess);
	}
	...
	private boolean internVersionCheck(String core, double actVersion, double minversion, double maxversion, YMessagesDialog mess) {
		...
		if ((minversion == -1 || minversion <= actVersion) && (maxversion == -1 || maxversion > actVersion)) {
			...
			mess.add(I18N.t("{0} benötigt mind. {1}. Es ist aber nur {2} installiert. Bitte update {3}.", info.getTitle(), minversion,
					actVersion, core), Level.INFO_INT);
		} else {
			mess.add(I18N.t("{0} benötigt max. {1}. Es ist schon {2} installiert. Bitte update {0}.", info.getTitle(), core, actVersion),
					Level.INFO_INT);
		}
	...
	/**
	 * @deprecated
	 * @see #isVersionInstall
	 */
	@Deprecated
	public boolean isMinVersionInstall(String id, double version, YMessagesDialog mess) {

		// has the version?
		return isVersionInstall(id, version, -1, mess);
	}
}
