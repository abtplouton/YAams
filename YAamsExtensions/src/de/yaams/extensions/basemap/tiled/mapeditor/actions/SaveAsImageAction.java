...
public class SaveAsImageAction extends AbstractAction {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5249081784874563180L;
	...
	private void saveMapImage(String filename) {
		...
		if (lastDot == -1) {
			JOptionPane.showMessageDialog(appFrame, "No file format specified.", "Error while saving map image", JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		try {
			...
			JOptionPane.showMessageDialog(appFrame, "Out of memory while creating image. Try increasing\n"
					+ "your maximum heap size or zooming out a bit.", "Out of memory", JOptionPane.ERROR_MESSAGE);
		}
	}
