...
public abstract class AbstractFileAction extends AbstractAction {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2487004191737479873L;
	...
	public final void actionPerformed(ActionEvent e) {
		if (editor.isModified()) {
			int ret = JOptionPane.showConfirmDialog(editor.getAppFrame(), SAVE_CHANGES_TEXT, SAVE_CHANGES_TITLE,
			...
			if (ret == JOptionPane.YES_OPTION) {
				...
				if (!saveAction.isSavingCancelled() && !editor.isModified()) {
					doPerformAction();
				}
	...
	/**
	 * Actually performs the action, in the confidence that any existing map has
	 * been either saved or discarded.
	 */
