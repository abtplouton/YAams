...
public class I18N {
	 ...
	 * LangFolder
	 */
	public static final File FOLDER = new File(YAamsCore.programPath, "lang");
	...
	public static void init() {
		...
		ExtentionManagement.add("form.options.info", new IExtension() {
			...
			public void work(HashMap<String, Object> objects) {
				...
				form.addElement("basic.lang", new FormInfo(I18N.t("Sprache"), getLangs().get(SLANG)));
		...
		ExtentionManagement.add("form.options.system", new IExtension() {
			...
			public void work(HashMap<String, Object> objects) {
				...
				form.addElement(
						"basic.lang",
						YSettingHelper
								.combo(null, I18N.t("Sprache"), "lang", "de", I18N.getLangIDs(), I18N.getLangNames())
								.setInfoTxt(
				...
				if (YLevel.IS_ADVANCED) {
					form.addHeader("lang", new FormHeader(I18N.t("Sprachenverwaltung"), "lang").setColumn(4));
					form.addElement("lang.import", new FormButton(I18N.t("Sprachdatei importieren"), "lang_add", new AE() {
						...
						public void run() {
							...
							form.addElement(
									...
									new FormInfo(
											...
											.setSorting(-1));
							form.addElement("basic.id", new FormTextField(I18N.t("Sprachcode"), "").setInfoTxt(I18N.t("zB de, en"))
									.addValidator(new ValidatorNotEmpty()).addValidator(new ValidatorBase() {

										@Override
										public void isValide(YMessagesDialog y) {
											if (new File(FOLDER, form.getContentAsString()).exists()) {
												y.add(I18N.t("Sprachcode {0} existiert schon. Wirklich überschreiben?"), Level.INFO_INT);
											}

										}
									}));
							form.addElement("basic.name",
									new FormTextField(I18N.t("Sprachname"), "").setInfoTxt(I18N.t("zB Deutsch, Englisch")));
							form.addElement("basic.author", new FormTextField(I18N.t("Autor"), SystemUtils.USER_NAME));
							form.addElement("basic.file", new FormFileSelectField(I18N.t("Datei"), null, JFileChooser.OPEN_DIALOG, false,
									false));

							// show it
							if (!YDialog.showForm(I18N.t("Sprachdatei importieren"), "lang_add", form)) {
								return;
							}

							// create folder
							File folder = new File(FOLDER, form.getElement("basic.id").getContentAsString());
							FileHelper.mkdirs(folder);

							// create info.xml
							HashMap<String, String> info = new HashMap<String, String>();
							info.put("title", form.getElement("basic.name").getContentAsString());
							info.put("author", form.getElement("basic.author").getContentAsString());
							FileHelper.saveXML(new File(folder, "info.xml"), info);

							// copy xml
							File source = new File(form.getElement("basic.file").getContentAsString());
							FileHelper.copy(source, new File(folder, source.getName()));

						}
					}));

					form.addElement("lang.export", new FormButton(I18N.t("Sprachdatei exportieren"), "lang_arrow", new AE() {
						...
						public void run() {
							...
							form.addElement(
									...
									new FormInfo(
											...
											.setSorting(-1));
							form.addElement("basic.file", new FormFileSelectField(I18N.t("Datei"), null, JFileChooser.SAVE_DIALOG, false,
									true));

							// show it
							if (!YDialog.showForm(I18N.t("Sprachdatei exportieren"), "lang_arrow", form)) {
								return;
							}

							// save xml
							FileHelper.saveXML(new File(form.getElement("basic.file").getContentAsString()), cache);
	...
	public static void loadLang(String id) {
		...
		for (File f : new File(FOLDER, id).listFiles()) {
			if (f.isFile() && f.canRead() && f.getName().endsWith(".xml") && !f.getName().equals("info.xml")) {
