...
import de.yaams.extensions.basemap.tiled.mapeditor.util.cutter.BorderTileCutter;
import de.yaams.extensions.basemap.tiled.mapeditor.util.cutter.TileCutter;
import de.yaams.extensions.basemap.tiled.mapeditor.widget.ColorButton;
...
public class NewTilesetDialog extends JDialog implements ChangeListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4597059067348829720L;
	...
	private void init() {
		...
		propsButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent actionEvent) {
				PropertiesDialog lpd = new PropertiesDialog(null, defaultSetProperties);
	...
	private void createSetAndDispose() {
		...
		if (tilebmpCheck.isSelected()) {
			...
			try {
				...
				newTileset.importTileBitmap(file, getCutter(width, height, spacing, margin));
			} catch (IOException e) {
				JOptionPane.showMessageDialog(this, e.getLocalizedMessage(), IMPORT_ERROR_MSG, JOptionPane.WARNING_MESSAGE);
			}
		}
	...
	private void chooseColorFromImage() {
		...
		try {
			...
			JOptionPane.showMessageDialog(getOwner(), IMG_LOAD_ERROR + " " + e.getLocalizedMessage(), COLOR_CHOOSE_ERROR_TITLE,
	...
	private void setUseTileBitmap(boolean value) {
		...
		/*
		 * cutterBox.setEnabled(value); cutterLabel.setEnabled(value);
		 */
