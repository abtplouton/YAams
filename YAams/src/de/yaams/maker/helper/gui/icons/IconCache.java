...
public class IconCache {
	...
	public final static String[] games = new String[19];
	...
	public static void init(YMessagesDialog ymd) {
		...
		final String[] icons = { "add", "open", "graph-opt", "ok", "dummy", "game", "del", "warn", "info", "yrgss", "debug", "opts",
				"close", "recall", "audio", "graphic", "monitor", "help", "copy", "paste", "selectAll", "cut", "disk", "plugin", "mail",
				"yaams", "home", "project", "folder", "arrow", "java", "control", "web", "database", "zoom-100", "zoom-actual", "zoom-in",
				"zoom-out", "cancel", "left", "right", "down", "up", "view", "edit", "reload", "update", "user", "restart", "pushnumber",
				"trash", "off", "on", "search", "mouse", "tab", "en", "setup", "wait", "ress", "archive", "fav", "lang", "beta", "lock",
				"unlock", "arrow_left", "yes", "no" };
		for (final String s : icons) {
			addPNG(IconCache.class, "ico/" + s);
		}
		...
		for (int i = 0, l = 17; i <= l; i++) {
			addPNG(IconCache.class, "game/game" + i);
			games[i + 1] = "game" + i;
		}
	}
	 ...
	 * @param ymd
	 */
	public static void loadSystmIcons(YMessagesDialog ymd) {
	...
	public static void addPNG(final Class c, final String... name) {
		...
		for (final String s : name) {
			...
			for (final int pixel : pixels) {
				if (c.getResource(s + pixel + ".png") != null) {
					add(nam, c, s + pixel + ".png");
				}
			}
		}
	}
